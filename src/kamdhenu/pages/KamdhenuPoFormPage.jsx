...
export default function KamdhenuPoFormPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setLoadError(err.message || 'Could not load the purchase order');
      } finally {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!canSubmit) {
      setFormError('Pick a site and PO date, and add at least one equipment row with a quantity.');
      return;
    }
    setSaving(true);
    setFormError('');
    ...
    try {
      if (isEdit) {
        ...
        toast.success(`Purchase order ${res.purchaseOrder?.poNumber || ''} created`.trim());
        navigate(
      ...
      toast.error(err.message || 'Could not save the purchase order');
      setFormError(err.message || 'Could not save the purchase order');
      setSaving(false);
    }
  };
    ...
    <div className="page">
      <div className="page__head">
        <div className="page__head-text">
          ...
          <h1 className="page__title">{isEdit ? 'Edit Purchase Order' : 'New Purchase Order'}</h1>
          <p className="page__subtitle">
            {isEdit ? 'Update the PO header and work-type quantities.' : 'Order work-type quantities for a site.'}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={submit} noValidate>
        <div className="panel">
          ...
          <div className="kerp-form-row">
            <div className="field">
              <label className="field__label">PO date</label>
        ...
        <div className="kerp-head-actions">
          ...
          <button type="submit" className="btn btn--sm" disabled={!canSubmit}>
            {saving ? <span className="spinner" /> : isEdit ? 'Save changes' : 'Create PO'}
          </button>
        </div>
      </form>
    </div>
